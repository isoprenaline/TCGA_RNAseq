...
draw_h_v <- function(exprSet,need_DEG,n='DEseq2',group_list,logFC_cutoff){
  ...
  g = ggplot(data=need_DEG, 
    ...
    geom_text_repel(aes(label = sign), box.padding = unit(0.3, "lines"), point.padding = unit(0.4, "lines"), show.legend = F, size = 2)
  print(g)
  ggsave(g,filename = paste0(n,'_volcano.pdf'),device = "pdf",width=20,height=20)
  dev.off()
}